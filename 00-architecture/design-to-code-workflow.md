...
每條 row 將直接 codegen 一條 Playwright E2E。
...
```
tools/visual-diff.ts
  ├─ Playwright 跑成品 section → 1440×900 截圖
...
```
tools/codegen-e2e.ts
  └─ 從 behavior/interactions.md 每條 row 產生一個 Playwright test
     - 找 selector
     - 觸發 action
...
1. 將 [/10-ux-design/visual-parity-workflow.md](../10-ux-design/visual-parity-workflow.md) 視為本工作流的「視覺軌」具體實作（已存在）
2. 新增同類文件 `10-ux-design/interaction-spec-workflow.md` 規範 `behavior/` 與 `data/` 細節（TODO）
3. 在 project repo 加 CI job：
   - `lint:design-spec` — 確認每個 section 三份 spec 都存在且不含 TBD
   - `e2e:codegen` — 從 interactions.md 自動產 Playwright tests
   - `visual-diff` — pixel parity gate
